...
import {
	ExitCalled, OpenEc0lintDocRequest, Status, StatusNotification
} from './shared/customMessages';

import { Validate, CodeActionsOnSaveMode } from './shared/settings';

import {
	CodeActions, ConfigData, Ec0lint, FixableProblem, Fixes, Problem, RuleMetaData, RuleSeverities,
...
Ec0lint.initialize(connection, documents, inferFilePath, loadNodeModule);
...
namespace ValidateNotification {
	export const type: NotificationType<TextDocument> = new NotificationType<TextDocument>('ec0lint/validate');
}
...
documents.onDidOpen(async (event) => {
	...
	const settings = await Ec0lint.resolveSettings(document);
...
documents.onDidClose(async (event) => {
	...
	const uri = document.uri;
	Ec0lint.removeSettings(uri);
	SaveRuleConfigs.remove(uri);
	CodeActions.remove(uri);
	Ec0lint.unregisterAsFormatter(document);
...
function environmentChanged() {
	Ec0lint.clearSettings();
	RuleSeverities.clear();
	SaveRuleConfigs.clear();
	Ec0lint.clearFormatters();
...
namespace CommandIds {
	export const applySingleFix: string = 'ec0lint.applySingleFix';
	export const applySuggestion: string = 'ec0lint.applySuggestion';
	export const applySameFixes: string = 'ec0lint.applySameFixes';
	export const applyAllFixes: string = 'ec0lint.applyAllFixes';
	export const applyDisableLine: string = 'ec0lint.applyDisableLine';
	export const applyDisableFile: string = 'ec0lint.applyDisableFile';
	export const openRuleDoc: string = 'ec0lint.openRuleDoc';
}

connection.onInitialize((params, _cancel, progress) => {
	progress.begin('Initializing Ec0lint Server');
	...
	if (clientCapabilities.textDocument?.codeAction?.codeActionLiteralSupport?.codeActionKind.valueSet !== undefined) {
		capabilities.codeActionProvider = {
			codeActionKinds: [CodeActionKind.QuickFix, `${CodeActionKind.SourceFixAll}.ec0lint`]
		};
	}
...
async function validateSingle(document: TextDocument, publishDiagnostics: boolean = true): Promise<void> {
	...
	try {
		...
		const diagnostics = await Ec0lint.validate(document, settings);
		...
		if (!settings.silent) {
			...
			for (const handler of Ec0lint.ErrorHandlers.single) {
			...
			connection.console.info(Ec0lint.ErrorHandlers.getMessage(err, document));
...
messageQueue.registerNotification(DidChangeWatchedFilesNotification.type, async (params) => {
	// A .ec0lintrc has change. No smartness here.
	// Simply revalidate all file.
	RuleMetaData.clear();
	Ec0lint.ErrorHandlers.clearNoConfigReported();
	Ec0lint.ErrorHandlers.clearMissingModuleReported();
	Ec0lint.clearSettings(); // config files can change plugins and parser.
	RuleSeverities.clear();
	SaveRuleConfigs.clear();
...
const Ec0lintSourceFixAll: string = `${CodeActionKind.SourceFixAll}.ec0lint`;
...
messageQueue.registerRequest(CodeActionRequest.type, async (params) => {
	...
	function createDisableLineTextEdit(textDocument: TextDocument, editInfo: Problem, indentationText: string): TextEdit {
		...
		if (editInfo.line - 1 > 0) {
			// Check previous line if there is a ec0lint-disable-next-line comment already present.
			...
			// specific line.
			const matchedLineDisable = new RegExp(`${lineComment} ec0lint-disable-next-line`).test(prevLine);
			...
			const matchedBlockDisable = new RegExp(`${blockComment[0]} ec0lint-disable-next-line`).test(prevLine);
		...
		if (commentStyle === 'block') {
			disableRuleContent = `${indentationText}${blockComment[0]} ec0lint-disable-next-line ${editInfo.ruleId} ${blockComment[1]}${EOL}`;
		} else { // commentStyle === 'line'
			disableRuleContent = `${indentationText}${lineComment} ec0lint-disable-next-line ${editInfo.ruleId}${EOL}`;
		}
	...
	function createDisableSameLineTextEdit(textDocument: TextDocument, editInfo: Problem): TextEdit {
		...
		const matchedLineDisable = new RegExp(`${lineComment} ec0lint-disable-line`).test(currentLine);
		const matchedBlockDisable = new RegExp(`${blockComment[0]} ec0lint-disable-line`).test(currentLine);
		if (matchedLineDisable) {
			...
			const commentStyle = settings.codeAction.disableRuleComment.commentStyle;
			disableRuleContent = commentStyle === 'line' ? ` ${lineComment} ec0lint-disable-line ${editInfo.ruleId}` : ` ${blockComment[0]} ec0lint-disable-line ${editInfo.ruleId} ${blockComment[1]}`;
	...
	function createDisableFileTextEdit(textDocument: TextDocument, editInfo: Problem): TextEdit {
		...
		return TextEdit.insert(Position.create(line, 0), `${block[0]} ec0lint-disable ${editInfo.ruleId} ${block[1]}${EOL}`);
	}
	...
	const settings = await Ec0lint.resolveSettings(textDocument);

	// The file is not validated at all or we couldn't load an ec0lint library for it.
	if (settings.validate !== Validate.on || !TextDocumentSettings.hasLibrary(settings)) {
		return result.all();
	}
	...
	const isSourceFixAll = (only === Ec0lintSourceFixAll || only === CodeActionKind.SourceFixAll);
		...
		if (isSourceFixAll) {
			...
			if (edits !== undefined) {
				result.fixAll.push(CodeAction.create(
					`Fix all fixable Ec0lint issues`,
...
async function computeAllFixes(identifier: VersionedTextDocumentIdentifier, mode: AllFixesMode): Promise<TextEdit[] | undefined> {
	...
	let start = Date.now();
	// Only use known fixes when running in onSave mode. See https://github.com/microsoft/vscode-ec0lint/issues/871
	// for details
	if (mode === AllFixesMode.onSave && settings.codeActionOnSave.mode === CodeActionsOnSaveMode.problems) {
		...
		return Ec0lint.withClass(async (ec0lintClass) => {
			...
			const content = fixes !== undefined
				...
				: originalContent;
			const reportResults = await ec0lintClass.lintText(content, { filePath });
			connection.tracer.log(`Computing all fixes took: ${Date.now() - start} ms.`);
...
messageQueue.registerRequest(ExecuteCommandRequest.type, async (params) => {
	...
	if (params.command === CommandIds.applyAllFixes) {
		...
		if ([CommandIds.applySingleFix, CommandIds.applyDisableLine, CommandIds.applyDisableFile].indexOf(params.command) !== -1) {
			...
			if (url) {
				void connection.sendRequest(OpenEc0lintDocRequest.type, { url });
			}
		} else {
...
connection.listen();
connection.console.info(`Ec0lint server running in node ${process.version}`);
